...
const options = () => {
    const stripePromise = loadStripe("pk_test_51NnfQeSDGyrzXvQ3gOcGK4oPdALVxIXTxKsZU1P76WF38DfgZiI1NrsCFdbGksBx05bpXakLQAhXU2LRq86OX6UM00HYXT0tkN")
            ...
            <div className='row p-2'>
                <p style={{'padding':'10px','display':'inlineFlex','color':'#fff'}}><i className='fa fa-arrow-left' onClick={()=>cancleorder()}/> Back</p>
                 {clientSecret && (
                    ...
                    <CheckoutForm packageId={subscriptionId}  orderId={!orderId?'':orderId}  paymentId={paymentId}/>
                    </Elements>
                )}
            </div>
            :
            <div className='row'>                
                <Link href='/subscription'><p style={{'color':'#fff','fontSize':'22px','padding':'10px 20px'}}>Back <i className='fa fa-angle-right'></i></p></Link>
                { payment.length > 0 && payment.map((x,index)=>{
                return(
                  <div className="inner" key={index}>
                        {x.logo === 'Stripe' || x.logo === 'Paytm' ? //Upilogo
                        <div className='innerchild'>
                        <Image className='image' src={ x.logo === 'Stripe' ? stripelogo : x.logo === 'Gpay' ? Gpay : x.logo === 'Paytm' ? paytmlogo : x.logo ==='Paypal' ? paypal : Upilogo } alt="pay" width="200" height="120" onClick={()=> createOrder(x._id,x.name)}/>
                        <div className='title'>
                          <h2>{x.name} <i className='fa fa-angle-right'></i></h2>
                          <p>{x.description}</p>
                        </div> 
                        </div>:
                        ''}
