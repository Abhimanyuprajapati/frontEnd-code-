  ...
  const handleepisode=()=>{
    ...
    .then(res=>{
      ...
      if(res.data.code === 75001){
        ...
        decrypt(res.data.key,'c4ba1f75da4ad97f')
              ...
              file: {
                hlsOptions: {
                  ...
                  xhrSetup: function(xhr, url) {
                      ...
                      if(url.includes('keyserver.filmcityi.app') && url.includes('getKey')){
                      var splitt = url.split('?');
                      //console.log(`${splitt[0]}?key=${finalDecrypted}&_id=${userId}`)
                      xhr.open('POST', `https://keyserver.filmcityi.app/getKey?key=${finalDecrypted}&_id=${userId}`)
